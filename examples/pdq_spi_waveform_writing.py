...
class PDQSPITEST(EnvExperiment):
    ...
    def build(self):
        ...
        self.setattr_device("ttl0")  # scope trigger
    ...
    def trigger(self):
        ...
        for frame in range(self.frames):
            self.pdq.set_frame(frame)
            self.ttl0.pulse(10*us)  # scope trigger
            self.pdq.set_config(clk2x=0, trigger=1, enable=1, aux_miso=1)
            self.pdq.set_config(clk2x=0, trigger=0, enable=1, aux_miso=1)
            delay(0.25*s)
            
 
    def prepare(self):
        self.chan_list, self.chan_data_list = self.pdq.program_host(self.wavesynth_example)
        self.pdq.program_host(self.wavesynth_example2)
